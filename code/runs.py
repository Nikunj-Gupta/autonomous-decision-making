    ...
    for seed in SEEDS: 
        for algo in ALGOS: 
            for explore in EXPLORATION_STRATEGIES: 
                if PARALLEL: os.system(f"python main.py --map {m} --algo {algo} --exploration_strategy {explore} --seed {seed} &") 
                else: os.system(f"python main.py --map {m} --algo {algo} --exploration_strategy {explore} --seed {seed}") 
